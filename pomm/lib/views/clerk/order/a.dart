...
class _OrderClerkPageState extends State<OrderClerkPage> {
  ...
  /// ✅ **Returns the filtered orders based on the selected tab**
  ...
  Widget _buildOrderCard(int index) {
    ...
    return Card(
      ...
      child: InkWell(
        onTap: () async {
          Cart cart = Cart.fromJson(order.toJson());
  ...
  void loadOrders() async {
    try {
      String clerkId = widget.clerk.clerkid.toString();
      final response = await http.get(
        Uri.parse(
          "${MyServerConfig.server}/pomm/php/load_order_clerkadmin.php?id=$clerkId",
        ),
      );
      ...
      if (response.statusCode == 200) {
        ...
        log("Failed to load orders. Status Code: ${response.statusCode}");
      }

...
class ReportMonthlyTable extends StatefulWidget {
  ...
  @override
  State<ReportMonthlyTable> createState() => _ReportMonthlyTableState();
}

class _ReportMonthlyTableState extends State<ReportMonthlyTable> {
  ...
  Widget build(BuildContext context) {
    final displayDate = DateFormat('MMMM yyyy').format(widget.selectedDate);
  ...
  Future<void> _fetchAllProductDetails() async {
    final formattedDate = DateFormat('yyyy-MM').format(widget.selectedDate);
    int page = 1;
    ...
    try {
      while (keepFetching) {
        final url = Uri.parse(
          "${MyServerConfig.server}/pomm/php/load_sales_report_table_monthly.php?date=$formattedDate&pageno=$page",
        );
  ...
  Future<void> printingMethod() async {
    final reportType = "monthly";
    final formattedDate = DateFormat('MMMM yyyy').format(widget.selectedDate);
